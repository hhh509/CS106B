...
bool connected(const Map<string, Set<string>>& roadNetwork,Set<string>& supplyLocations)
{
    for(string s : roadNetwork){
        if(!isCovered(s,roadNetwork,supplyLocations)){
            return false;
        }
    }
    return true;
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if(numCities < 0)
        return false;
    if(numCities >= 0 && connected(roadNetwork,supplyLocations)){
        return true;
    }
    for(string s : roadNetwork){
        if(!isCovered(s,roadNetwork,supplyLocations)){
            supplyLocations.add(s);
            if(canBeMadeDisasterReady(roadNetwork,numCities-1,supplyLocations))
                return true;
            supplyLocations.remove(s);
            for(string i : roadNetwork[s]){
                supplyLocations.add(i);
                if(canBeMadeDisasterReady(roadNetwork,numCities-1,supplyLocations))
                    return true;
                supplyLocations.remove(i);
            }
        }
    }
    return false;
