...
bool helper(Map<string, Set<string>>& possibleLinks, Set<Pair>& matching,int num,Set<string> q) {
    /* TODO: Delete this comment and these remaining lines, then implement this function. */
    if(num%2){
        return false;
    }
    if(matching.size()==num/2)
        return true;
    string s = possibleLinks.firstKey();
    Set<string> set = possibleLinks[s];
    if(set.isEmpty())
        return false;
    for(string i : set){
        if(q.contains(i))
            continue;
        Pair p(s,i);
        Set<string> set1 = possibleLinks[i];
        matching.add(p);
        q.add(s);
        q.add(i);
        possibleLinks.remove(s);
        possibleLinks.remove(i);
        bool h = helper(possibleLinks,matching,num,q);
        if(h)
            return true;
        possibleLinks.put(s,set);
        possibleLinks.put(i,set1);
        matching.remove(p);
        q.remove(s);
        q.remove(i);
    }
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching){
    Map<string,Set<string>> p = possibleLinks;
    int num = p.size();
    Set<string> s;
    return helper(p,matching,num,s);
}

bool hasmatching(Map<string,Map<string,int>>& pl, Set<string>& e){
    for(string s : pl){
        Map<string,int> value = pl[s];
        for(string p : value){
            if(!e.contains(p))
                return true;
        }
...
Set<Pair> r = {};
int val = 0;
void helper1(const Map<string, Map<string, int>>& possibleLinks,Map<string, Map<string, int>>& pl, Set<Pair>& set,Set<string>& e){
    if(!hasmatching(pl,e))
    {
        if(val > m){
            r = set;
    ...
    /*string key = pl.firstKey();
    Map<string,int> value = pl[key];
    for(string res : value){
        if(e.contains(res))
            continue;
        Pair p(key,res);
        set.add(p);
        Map<string,int> value1 = pl[res];
        int v = pl[key][res];
        pl.remove(key);
        pl.remove(res);
        e.add(key);
    ...
    for(string key : possibleLinks){
        ...
        for(string res : value){
            ...
            Map<string,int> value1 = pl[res];
            int v = pl[key][res];
            Map<string, Map<string, int>> map = pl;
            map.remove(key);
            map.remove(res);
            e.add(key);
            e.add(res);
            val += v;
            helper1(possibleLinks,map,set,e);
            val -= v;
            map.put(key,value);
            map.put(res,value1);
            set.remove(p);
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    /* TODO: Delete this comment and these remaining lines, then implement this function. */
    Set<Pair> set;
    Map<string, Map<string,int>> pl = possibleLinks;
    Set<string> p;
    m = 0;
    val = 0;
    r.clear();
    helper1(possibleLinks,pl,set,p);
    return r;
}
